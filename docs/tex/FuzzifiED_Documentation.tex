...
\renewcommand{\algorithmiccomment}[1]{\hfill{\blendcolors*{!50}\textcolor{Black}{$\triangleright$ \textit{#1}}}}
...
After generating the sparse matrix, the method \lstinline|GetEigensystem| calls the Arpack package to calculate its lowest eigenstates. 
...
In the example of Ising model, to calculate the OPE coefficient $f_{\sigma\sigma\epsilon}=\langle \sigma|n^z_{00}|\epsilon\rangle/\langle \sigma|n^z_{00}|0\rangle$, one need to first calculate the eigenstates in the $\mathscr{Z}=-$ sector
\begin{lstlisting}

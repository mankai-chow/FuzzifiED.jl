...
N_e&=∑_{mf}n_{mf}\\\\
L_z&=∑_{mf}mn_{mf}\\\\
S_{z,i}&=∑_m (n_{m,i}-n_{m,i+N_f/2})&i&=1,…,N_f/2
\\end{aligned}
```
...
Return the basis with conserved parity ``\\mathscr{P}``, ``π``-rotation along ``y``-axis ``\\mathscr{R}``, and flavour symmetries ``𝒵_i,\\mathscr{X}_i`` from the configurations already generated. Quantum numbers set to zero signify that they are not conserved. 

```math
\\begin{aligned}
\\mathscr{P}&:c^{†}_{mf}↦ c_{m,f+N_f/2},&c^{†}_{m,f+N_f/2}&↦-c_{m,f}\\\\
\\mathscr{R}&:c^{†}_{mf}↦ c^{†}_{-m,f}\\\\
𝒵_i&:c^{†}_{mi}↦ c^{†}_{m,i+N_f/2}&c^{†}_{m,i+N_f/2}&↦-c^{†}_{m,i},&i=1,2,…,N_f/2\\\\
\\mathscr{X}_i&:c^{†}_{m,2i-1}\\leftrightarrow c^{†}_{m,2i}&c^{†}_{m,2i-1+N_f/2}&\\leftrightarrow c^{†}_{m,2i+N_f/2},&i=1,2,…,N_f/4\\\\
\\end{aligned}
```
Note that sometimes some ``𝒵_i`` and ``\\mathscr{X}_i`` cannot be implemented at the same time. 
...
- `qn_z :: Vector{Int64}` is ``N_f/2`` numbers that give ``𝒵_i`` flavour symmetry. Facultative, all 0 by default.
...
```math
∑_{m_1m_2m_3m_4ff'}2U_{m_1m_2m_3m_4}c^{†}_{m_1f}c^{†}_{m_2,f+N_f/2}c_{m_3 f'+N_f/2}c_{m_4f'}
```
...
function GetSpnPairIntTerms(nm :: Int64, nf :: Int64 ; ps_pot :: Vector{<:Number} = [1.])
    ...
    return GetPairIntTerms(nm, nf ; ps_pot, mat_a = Ω)
end

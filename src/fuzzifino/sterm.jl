...
A `STerm` object records a STerm that looks like ``Ua^{(p_1)}_{o_1}a^{(p_2)}_{o_2}… a^{(p_l)}_{o_l}`` in an operator, where positive ``o`` denotes fermions and negative ``o`` denotes bosons
...
It can be generated by the function
    ...
    function STerm(coeff :: ComplexF64, cstr :: Vector{Int64}) :: ComplexF64 
"""
mutable struct STerm 
    coeff :: ComplexF64 
    cstr :: Vector{Int64}
